...
5. 取り消したいソフトウェア購入を見つけて、 **[キャンセル]** を選択します。 ダイアログ ボックスが表示されます。

6. 注文番号 ドロップダウンリストから、キャンセルする正しい注文 ID 番号を選択し、**送信** を選択します。

7. 顧客に対して複数の注文をキャンセルする場合は、一意の注文 ID 番号ごとに手順 4. ~ 6. をもう一度実行する必要があります。 (注文または注文 ID 番号の詳細については、顧客の**注文履歴**ページを参照してください)。

8. 注文を取り消すと、パートナーセンターから他の情報が表示される場合もあります (注文番号のドロップダウンリストに表示されます)。 次の情報が含まれます。
