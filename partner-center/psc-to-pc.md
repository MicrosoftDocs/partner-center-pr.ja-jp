...
>[!Note]
> ユーザーの移行は、会社の [アカウント管理者](permissions-overview.md#manage-mpn-membership-and-your-company) のみが実行できます。 アカウント管理者の役割を持っていない場合は、ユーザーの移行ウィザードを使用して、ユーザーアカウントのセットアップを支援するアカウント管理者を見つけます。 ユーザーの移行機能は、2020年11月18日からご利用いただけます。
...
一部の取引のみが、PSC から PC への移行に適しています。 この移行ウィザードは、パートナーがパートナーセンターに取引をする際に役立つように構築されています。パートナーセンターでは、顧客と積極的に連携して取引を成約させることができます。 **有効なパートナーアカウントの詳細 (有効な MPN ID) を使用してオープン状態にある取引だけを対象としており、参加していない案件は移行の対象となります。**
...
>[!Important]
>アクティブな取引は PC に移行されません。 2021年3月31日までは、取引を終了して登録することができます。
