...
-   サインイン、**パートナー センター**で、**の移行に**(既存の) アカウント (遷移は 1 つ) をすべての顧客に注意してくださいし、ユーザー向けのすべてのサービスをプロビジョニングします。
...
1.  サインイン、**パートナー センター**で、**の移行に**(に移行しているもの) を (新規) アカウントから顧客リストに移動**顧客**します。

2.  [顧客] を選びます。

3.  **[再販業者関係の要求]** をクリックします。 顧客に表示する既定のメール メッセージが表示されます。 このメッセージには、新しいパートナー センター アカウントに対して一意の URL と組織 ID が含まれています。

4.  **顧客の操作:** この URL にアクセス、アクティブな顧客を移行することを確認します。 この URL を開くと、顧客は、Office 365 ポータルにサインインするよう求められます。 顧客は、Azure や Office 365 管理ポータルにアクセスするために使用する組織 ID と同じ組織 ID を使用してサインインします。
...
>[!**注**]、顧客の下にあるサブスクリプションを無効にすると、顧客リスト内の顧客の外観が変更されません。 現在、一覧から顧客を削除するオプションはありません。 パートナーは、今後、**移行元**アカウントからこれらの顧客にサブスクリプションを追加しないでください。
...
1.  クリックして**顧客**既存の構造内の顧客の一覧を表示するパートナー センターにします。
