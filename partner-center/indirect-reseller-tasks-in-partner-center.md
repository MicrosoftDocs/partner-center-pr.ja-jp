...
4. Microsoft Partner Network ID (MPN ID) を新しいパートナー センター アカウントに関連付ける方法を確認します。 Microsoft ではお客様の会社の MPN ID を新しいパートナー センター アカウントに関連付ける必要があります。これにより、お客様はパートナー センターでメンバーシップ特典やインセンティブを管理できるようになります。 **[サインイン]** を選びます。
...
7. インセンティブ、メンバーシップの特典、コンピテンシーにリンクされている MPN ID を使用して、会社のプロファイルを選択します。 その後、この情報をパートナー センター アカウントに移動できます。 **[続行]** を選びます。

   使用する MPN ID がこのページに表示されていない場合は、 **[キャンセル]** を選び、使用する会社プロファイルのユーザー名とパスワードを使ってサインインし直します。

   > [!IMPORTANT]  
   > Azure AD テナントと同じ国または地域にある、アクティブな会社プロファイルに関連付けられた MPN ID を選択してください。

8. 会社の法人プロファイル、第一連絡先担当者情報、カスタマー サポートに関する情報を入力または更新します。 **[今すぐ登録する]** を選びます。

   第一連絡先担当者には、貴社のアプリケーションについて Microsoft から問い合わせることができる社内の担当者を指定してください。 第一連絡先担当者には、ご自身または貴社の別のユーザーを指定できます。 注: Web ベースの電子メール サービスで取得したメール アドレスは使用できません (Gmail、Yahoo! メール、iCloud メール、AOL メール、Outlook メールなど)。
