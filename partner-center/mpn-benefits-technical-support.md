...
手順 1. windows 10 を選択してダミー情報を入力する

手順2アクセス ID と契約 ID を使用して、サポート契約を追加します。 オンプレミスのサポートについては、パートナーサポート契約が表示されます。これには、開始日、終了日、残りのインシデントが含まれます。 [使用できないサポートプランを表示しますか?] を展開します。 パートナーの署名情報を表示するセクション。 ここに表示されるのは、契約の有効期限が切れた場合、インシデントが残っていない場合、または古いオンプレミス製品を選択した場合のみです (サポート製品については、 [aka.ms/n-1](https://aka.ms/n-1) を参照してください)。
...
*"このアクセス ID は既にアカウントに関連付けられています。新しいアクセス ID を取得するには、テクニカルアカウントマネージャーに問い合わせてください。 "* **このエラーメッセージは無効なため、無視する必要があります。** エラーを無効にし、役に立つ診断エラーメッセージに置き換えるように要求しました。 

上記の "アクセス ID は既に関連付けられています..." と表示されます。 Azure portal を使用するときに、サポート契約を1つのアカウントに複数回関連付けると、エラーが発生します。 以下のシナリオを参照してください。
